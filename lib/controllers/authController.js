...
module.exports = function(server) {
    server.post('/auth/signup', function(req, res) {
        req.form.complete(function(err, fields, files) {
            ...
            db.Users.getAnonymous(req.cookies.anonymous, function(user) {
            db.Users.checkName(fields.username, function(available) {
                
                if ((!available && !user) || (!available && user && user.name != fields.username)) {
                ...
                savePermanentUser(user, fields.username, fields.email, fields.password, function() {
                    req.fields = fields;
...
function savePermanentUser(user, name, email, password, callback) {
    if (user) {
        saveData(user);
    } else {
