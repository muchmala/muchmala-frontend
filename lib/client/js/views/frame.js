...
FrameProto.render = function() {
    ...
    var border = 1;
    var offset = this.step + size + border;
    ...
    this.element.find('.corner').width(this.step + size).height(this.step + size);
};
...
function HorizontalTile(settings) {
    ...
    this.height = this.step * 2 + this.size;
}
...
function VerticalTile(settings) {
    ...
    this.width = this.step * 2 + this.size;
}
