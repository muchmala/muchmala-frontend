    ...
    (new Puzz.Views.Blinker([[400, 200]], [
        function() {
            _.each(pieces, function(piece) {
                piece.createBorder('blink', '#e00', true);
            });
        },
        function() {
            _.each(pieces, function(piece) {
...
Proto.addPiece = function(data) {
    var rectSize = this.puzzleModel.get('rectSize');
    var frameSize = this.puzzleModel.get('frameSize');
    var xCoord = data.x * (rectSize + 1) + frameSize + 1;
    var yCoord = data.y * (rectSize + 1) + frameSize + 1;
