$(function() {
    ...
    var loadPuzzle = (function() {
        ...
        function calcLoading(loadedCount) {
            ...
            return Math.floor(objectsLoaded * 100 / (5 + rows * cols));
        }
        ...
        return flow.define(function() {
            ...
            load.frame(puzzleModel.get('pieceSize'), this);
        }, function(sprite) {
            viewport.loading(calcLoading(1));
            
            // --------------------------------------
            // THERE WE BUILD PUZZLE FRAME
            // --------------------------------------
            var frame = new Puzz.Views.Frame({
                element: viewport.frame,
                rectSize: viewport.rectSize,
    ...
    function enablePuzzle() {
        ...
        var blower = new Puzz.Views.Blower(userModel, viewport.puzzle, viewport.pieceSize, viewport.rectSize);
        var tooltiper = new Puzz.Views.Tooltiper(viewport.puzzle, viewport.pieceSize, viewport.rectSize);
        ...
        server.bind(MESSAGES.lockPiece, function(locked) {
            ...
            if (locked.userName == userModel.get('name')) {
                ...
                tooltiper.add(x, y, locked.userName);
                piece.lock();
            }
        });
